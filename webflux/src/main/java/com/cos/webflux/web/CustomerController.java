...
public class CustomerController {
	...
	@PutMapping(value="/customer/{id}",produces = MediaType.APPLICATION_STREAM_JSON_VALUE)
	public Mono<Customer> update(@PathVariable Long id, @RequestBody Customer customer){
		customer.setId(id);
		return customerRepository.save(customer);
		// 두번째 방법 - mUpdate() 사용
		//return customerRepository.mUpdate(customer.getLastName(), customer.getFirstName(), customer.getId());
	}
}
